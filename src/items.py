import progression,rom,args,rpg,categories
...
class Item:
  ...
  def load(self,vanilla):#TODO
    self.index=int.from_bytes(rom.read(vanilla,self.indexaddress),byteorder='little')
    self.price=int.from_bytes(rom.read(vanilla,self.priceaddress,4),byteorder='little')
...
def populate(vanilla,generated):
  ...
  for l in loot:
    if rpg.chancein(3):
      ...
      rom.write(int(categoryindex.index(c)).to_bytes(2,byteorder='little'),l.index,generated)
      rom.write(int(c.category).to_bytes(2,byteorder='little'),l.category,generated)

def generate(rewards,vanilla,generated):
  for category in [armor,consumables,weapons]:
